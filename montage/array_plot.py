...
if len(sys.argv)<2:
    sys.exit("Syntax: python array_plot.py input.dat")
...
for f in listconf:
	...
	n=f.split("_")
	print "\033[F"+"                                                                                    "
	print "\033[F"+"Processing file f_"+n[-2]+"_"+n[-3]+"_"+n[-4]+" ("+str(counter+1)+" of "+str(len(listconf))+")"
    	sys.stdout.flush()
	...
	fig.savefig("finalconfigurations/"+"f_"+str(listg.index(float(n[-4])))+"_"+str(listc.index(float(n[-3])))+"_"+str(listn.index(int(n[-2])))+".png",pad_inches=0,dpi=200)
